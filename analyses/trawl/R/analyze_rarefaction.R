...
trawl_mob_stats$groups_stats %>% dplyr::filter(index == "S_asymp") %>% dplyr::select(group, index, value) %>%
  dplyr::mutate(Date = as.Date(paste(group,"01","01", sep = "-"), format = "%Y-%m-%d")) %>%
  ggplot() +
  geom_point(aes(x = Date, y = value))+
  geom_path(aes(x = Date, y = value))

trawl_mob_stats$groups_stats %>% dplyr::filter(index == "S_n") %>% dplyr::select(group, index, effort, value) %>%
  dplyr::mutate(Date = as.Date(paste(group,"01","01", sep = "-"), format = "%Y-%m-%d")) %>%
  ggplot() +
  geom_point(aes(x = Date, y = value, group = effort))+
  geom_path(aes(x = Date, y = value, group = effort))

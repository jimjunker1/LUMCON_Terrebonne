...
```{r turnover plot ,echo = FALSE, warning=FALSE, message=FALSE, fig.cap = "Figure 2. Total species "}
trawl_yr_turnover %>%
  ggplot()+
...
Species turnover was likley confined to species with moderate to low relative abundances. The effective number of highly abundant species, *^2^D*, was variable but stable through time with calculated values *ca.* `r qD_df %>% dplyr::filter(q_order == 'q2') %>% dplyr::summarise(across(median, ~median(.x ,na.rm = TRUE))) %>% round(2)` $\pm$ `r qD_df %>% dplyr::filter(q_order == 'q2') %>% dplyr::summarise(across(median, ~sd(.x, na.rm = TRUE))) %>% round(1)` (mean $\pm$ 1 SD) (range: `r qD_df %>% dplyr::filter(q_order == 'q2')%>% slice_min(median) %>% dplyr::select(median) %>% round(2)` to  `r qD_df %>% dplyr::filter(q_order == 'q2') %>% slice_max(median) %>% dplyr::select(median) %>% round(1)`; Figure 3 __*supplemental?*__). In contrast, the percentage of rare species increased through time from `r rarity %>% slice_min(group) %>% dplyr::select(median) %>% round(2)` (`r rarity %>% slice_min(group) %>% dplyr::select(lower) %>% round(2)` -- `r rarity %>% slice_min(group) %>% dplyr::select(upper) %>% round(2)`, 95% percentile interval [PI]) to `r rarity %>% slice_max(group) %>% dplyr::select(median) %>% round(2)` `r rarity %>% slice_max(group) %>% dplyr::select(lower) %>% round(2)` -- `r rarity %>% slice_max(group) %>% dplyr::select(upper) %>% round(2)`, 95% PI; Figure 3A). Additionally, the normalized evenness in species relative abundances showed a consistent decline through time from a median value of `r annual_evenness_df %>% dplyr::filter(measure == 'gini_norm') %>% slice_min(Date) %>% dplyr::select(median) %>% round(2)` in `r min(annual_evenness_df$year,na.rm = TRUE)` (`r annual_evenness_df %>% dplyr::filter(measure == 'gini_norm') %>% slice_min(Date) %>% dplyr::select(quant2.5) %>% round(2)` -- `r annual_evenness_df %>% dplyr::filter(measure == 'gini_norm') %>% slice_min(Date) %>% dplyr::select(quant97.5) %>% round(2)`, 95% PI) to `r annual_evenness_df %>% dplyr::filter(measure=='gini_norm') %>% slice_max(Date) %>% dplyr::select(median) %>% round(2)` (`r annual_evenness_df %>% dplyr::filter(measure == 'gini_norm') %>% slice_max(Date) %>% dplyr::select(quant2.5) %>% round(2)` -- `r annual_evenness_df %>% dplyr::filter(measure == 'gini_norm') %>% slice_max(Date) %>% dplyr::select(quant97.5) %>% round(2)`, 95% PI) in `r max(annual_evenness_df$year, na.rm = TRUE)`.

```{r diversity2 plot, echo = FALSE, warning=FALSE, message=FALSE, fig.cap="Figure 3. Temporal trends in order of diversity q = 2, ^2^D highlight relative stability in the number of dominant species in the community through time. ^2^D is equivalent to 1/Simpson's diversity index and captures the effective number of highly abundant species in the community."}
qD_df %>%
...
```{r evenness, echo = FALSE, warning=FALSE, message=FALSE, fig.cap= "Figure 4." }
...
annual_evenness_df %>%
  ...
  scale_y_continuous(name = "Normalized Gini coefficient", limits = c(0,NA), expand = c(0.01,0.01))+
  scale_x_date(date_breaks = 'year', limits = c(as.Date("2007-01-01"),as.Date("2019-06-01")),
               date_labels = "%Y")+
...
group_stats %>%
  ...
  ggplot()+
  geom_point(aes(x =group, y = median), size = 3, shape = 22)+
  geom_path(aes(x = group, y = median), size = 1.1)+
  geom_ribbon(aes(x = group, ymin = lower, ymax = upper), alpha = 0.3)+
  scale_y_continuous(name = "Percent rare species (%)", limits = c(70,100), expand = c(0.01,0.01))+

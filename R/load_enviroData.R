...
regional_temp_paths = list.files(path = "./data/environment/", "^X.*.nc", full.names = TRUE)
...
full_dates = seq.Date(from = as.Date("2008-01-01"), to = as.Date("2020-12-31"), by = 1) %>% data.frame %>% setNames('Date')
...
## salinity
regional_salinity_paths = list.files(path = "./data/environment/","^aggregate.*.nc", full.names = TRUE)

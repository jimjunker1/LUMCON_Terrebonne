...
source(here::here("sub-project/trawl/R/Lorenz.R"))
source(here::here("sub-projects/trawl/R/Evenness.R"))


TB_trawl_data = TB_trawl_data %>%
   dplyr::filter(year %ni% c("2007","2020","2021"))
...
Species names were checked for typos and confirmed using the *gni_parse* function from the *taxize* package [@chamberlain2020] and further standardized to consistent taxonomic resolution for entire data series (Supplemental materials). We measured multiple aspects of species richness, first to track the uncorrected accumulation of species over time we filtered species from the data set after first occurrence. We then calculated two measures of species richness, accounting for differences in abundance among trawls. The first measure was asymptotic species richness, *S~asymp~*, using the Chao1 estimator [@chao1984][@chao1987]. This metric is highly correlated with measured richness, *S* [@mcgill2011], but attempts to extrapolate the total species richness based on incomplete sampling of the community. Second, we estimated effort-controlled rarefied species richness, *S~n~*, which provides the expected number of species given a defined number of *n* individuals sampled [@gotelli2001]. For *S~n~* estimates for trawls, *n* was calculated by the minimum number of individuals within a trawl multiplied by `r n_year`. We calculated *S~asymp~* and *S~n~* using the mobr package [@mcglinn2019][@mcglinn2021a].

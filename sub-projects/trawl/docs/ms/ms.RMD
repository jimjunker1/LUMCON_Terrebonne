...
To detect trends in species richness and other dimensions of biodiversity (e.g., turnover, evenness, etc.) we used generalized additive models (GAMs) with a bayesian implementations using the `mgcv` [@wood2011] and  `brms` packages in R [@burkner2017; @burkner2018]. Full models included smoothing spline terms for both trawl date and season as:
$$y = \beta_0 + f_{season}(x_1) + f_{time}(x_2) + \varepsilon,\;\;\; \varepsilon ~ N(0, \sigma^{2} \Lambda) $$
where, $y$ is the response variable (e.g., richness, turnover, evenness, etc.), $\beta_0$ is an intercept $f_{season}$ and $f_{time}$ represent seaonsal and trawl data smooth functions for month of year and day since beginning of the series, January 1, 2009. Seasonal terms were fit with cyclic cubic regression splines with smoothing dimension *k* = 12 to allow for continuity between December and January among model years. Models were 

# Results

```{r env data, echo=FALSE, warning=FALSE, message=FALSE}
...
Consistent with the pattern of continued accumulation of new species throughout the sampling series and relatively stable effort-controlled richness, species gains (`r round(mean(gains$value, na.rm = TRUE),2)` $\pm$ `r round(sd(gains$value, na.rm = TRUE),2)` SD) and losses (`r round(mean(losses$value, na.rm = TRUE),2)` $\pm$ `r round(sd(losses$value, na.rm = TRUE),2)` SD) generally balanced over time (Figure 1b). Total turnover was variable but relatively stable (`r round(mean(total$value, na.rm = TRUE),2)` $\pm$ `r round(sd(total$value, na.rm = TRUE),2)` SD) and exhibited no detectable trend or coherent seasonal component. Species turnover was likely confined to species with moderate relative abundances. While we observed declines in the effective number of species for highly rare (^0.1^D; population-level effects `r summary(q0.1_bayes_full_mod)[['fixed']]['sdate_id_1','Estimate']`, 95% CI: `r summary(q0.1_bayes_full_mod)[['fixed']]['sdate_id_1','l-95% CI']`--`r `summary(q0.1_bayes_full_mod)[['fixed']]['sdate_id_1','u-95% CI']`) and highly abundant species (^2^D; `r summary(q2_bayes_full_mod)[['fixed']]['sdate_id_1','Estimate']`, 95% CI: `r summary(q02_bayes_full_mod)[['fixed']]['sdate_id_1','l-95% CI']`--`r `summary(q2_bayes_full_mod)[['fixed']]['sdate_id_1','u-95% CI']`), declines were greater in the for diversity orders describing moderately abundant species (^0.5^D; `r summary(q0.5_bayes_full_mod)[['fixed']]['sdate_id_1','Estimate']`, 95% CI: `r summary(q0.5_bayes_full_mod)[['fixed']]['sdate_id_1','l-95% CI']`--`r `summary(q0.5_bayes_full_mod)[['fixed']]['sdate_id_1','u-95% CI']` and ^1^D; `r summary(q1_bayes_full_mod)[['fixed']]['sdate_id_1','Estimate']`, 95% CI: `r summary(q1_bayes_full_mod)[['fixed']]['sdate_id_1','l-95% CI']`--`r `summary(q1_bayes_full_mod)[['fixed']]['sdate_id_1','u-95% CI']`,  Figure 2).

These changes in the distribution of relative abundances within the community led to an ultimate decrease in the community evenness through time. In contrast, the percentage of rare species increased slightly through time from `r rarity %>% slice_min(year) %>% dplyr::select(median) %>% round(2)` to `r rarity %>% slice_max(year) %>% dplyr::select(median) %>% round(2)`. Additionally, the evenness in species abundances showed a clear decline through time from a median value of `r annual_evenness_df %>% dplyr::filter(measure == 'gini_norm') %>% slice_min(Date) %>% dplyr::select(median) %>% round(2)` in `r min(annual_evenness_df$year,na.rm = TRUE)` to `r annual_evenness_df %>% dplyr::filter(measure=='gini_norm') %>% slice_max(Date) %>% dplyr::select(median) %>% round(2)` in `r max(annual_evenness_df$year, na.rm = TRUE)`.

```{r diversity2 plot, echo = FALSE, warning=FALSE, message=FALSE, fig.cap="Figure 2. Temporal trends in order of diversity q = 2, ^2^D highlight relative stability in the number of dominant species in the community through time. ^2^D is equivalent to 1/Simpson's diversity index and captures the effective number of highly abundant species in the community.", include=FALSE}#### patterns of Hill diversity through time}
## hill 
...
hill_time = vctrs::vec_c(hill_time0,hill_time0.5,hill_time1,hill_time2) %>%
  group_by(Date, date_id) %>%
...
ceff_full = vctrs::vec_c(ceff01_df, ceff0.5_df, ceff1_df, ceff2_df) %>%
...
```{r spp contributions, echo=FALSE, warning=FALSE, message=FALSE}
...
```{r supplemental figure 1, echo=FALSE, warning=FALSE, message=FALSE}

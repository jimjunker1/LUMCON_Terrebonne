...
# # create bootstrapped data set to estimate variability
# TB_trawl_boots = TB_trawl_data %>%
#   group_by(year) %>%
#   modelr::bootstrap(TB_trawl_data)
...
To test for changes in species richness over time, we fit bayesian implementations of generalized additive models (GAMs) using the `mgcv` [@wood2011] and  `brms` package in R [@burkner2017; @burkner2018]. 
...
trawl_mth_turnover = purrr::map(c('appearance','disappearance','total'),
                                     ~codyn::turnover(df = TB_trawl_taxasite_long,
                                                      time.var = 'date_id',
...
```{r turnover plot ,echo = FALSE, warning=FALSE, message=FALSE, fig.cap = "Figure 2. Total species turnover and partitioning of species gains and losses over time."}
trawl_mth_turnover %>%
  na.omit %>%
  ggplot()+
